...
{
	const auto lineStart = object1->getPosition();
	const auto lineEnd = object1->getPosition() + object1->getCurrentDirection() * 100.0f;
	// aabb
	...
	const auto boxStart = object2->getPosition() - glm::vec2(halfBoxWidth, halfBoxHeight);
...
{
	// circle
	const auto circleCentre = object1->getPosition();
	...
	const auto boxStart = object2->getPosition() - glm::vec2(boxWidth * 0.5f, boxHeight * 0.5f);
	...
	if (circleAABBsquaredDistance(circleCentre, circleRadius, boxStart, boxWidth, boxHeight) <= (circleRadius * circleRadius))
	{
		if (!object2->getIsColliding()) {
			...
			const auto attackVector = object1->getPosition() - object2->getPosition();
